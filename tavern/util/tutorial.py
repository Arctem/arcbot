...
def tutorial_lock(function):
    def locked_function(self, arcuser, channel, args):
        ...
        if not tavern:
            self.say(channel, '{}: Please name your tavern with ".tavern name <tavern name>" first.'.format(arcuser.base.nick))
            return
        if not tavern.resident_hero:
            self.say(channel, '{}: Please hire your first hero with ".tavern hire <hero name>" first.'.format(arcuser.base.nick))
            return
